...
std::string prepare_custom_config(std::string path, gboolean vim_mark) {
    ...
    output_file << "# This file was automatically generated by github.com/marcospb19/termite\n"
                << "# You should not be editing it, see the README for more information\n";
    ...
    if (vim_mark) {
        output_file << "# vim: ft=dosini cms=#%s\n";
    }
...
static void load_config(GtkWindow *window, VteTerminal *vte, GtkWidget *scrollbar,
                        ...
                        bool *show_scrollbar, char* custom_config_file_path, gboolean vim_mark) {
    ...
    if (info->config_file or custom_config_file_path) {
        ...
        if (custom_config_file_path) {
            ...
            std::string custom_path = prepare_custom_config(std::string(custom_config_file_path), vim_mark);
...
int main(int argc, char **argv) {
    ...
    gboolean version = FALSE, hold = FALSE, vim_mark = FALSE;
    ...
    const GOptionEntry entries[] = {
        ...
        {"vim", 'V', 0, G_OPTION_ARG_NONE, &vim_mark, "Append vi syntax marker to final custom config file", nullptr},
    ...
    load_config(GTK_WINDOW(window), vte, scrollbar, hbox, &info.config,
                icon ? nullptr : &icon, &show_scrollbar, custom_config_file_path, vim_mark);

    reload_config = [&]{
        load_config(GTK_WINDOW(window), vte, scrollbar, hbox, &info.config,
                    nullptr, nullptr, custom_config_file_path, vim_mark);
    };
